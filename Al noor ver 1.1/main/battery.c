...
/* Pointer to main.c g_playing — alerts are suppressed while a story plays */
...
/* No-battery detection: if ADC consistently reads very low, pin is floating */
...
static void _check_alerts(int pct)
{
    ...
    /* Skip alerts if no battery detected (floating ADC pin) */
    if (s_no_battery) return;

    /* Skip alerts if a story/audio is currently playing — never interrupt */
    if (s_playing_flag && *s_playing_flag) return;
...
static void _battery_task(void *arg)
{
    while (1) {
        ...
        /* No-battery detection: if battery voltage is below 2500mV
         * (min Li-ion is ~3000mV) for 3 consecutive readings,
         * the ADC pin is likely floating — no battery connected.
         * USB power (5V) through voltage divider would read ~4100mV,
         * so a low reading means nothing is driving the pin. */
        if (bat_mv < 2500) {
        ...
        ESP_LOGI(TAG, "Battery: %d%% [%s]  %d mV  (ADC pin: %d mV)%s",
