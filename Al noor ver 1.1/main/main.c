 ...
 *
 * FIX C — Volume is interrupt-driven, works during any audio playback
 *   Root cause: volume buttons were polled in the 10ms main loop.
 *   audio_volume_up/down called announce_volume_percent() which called
 *   audio_play_file() directly — but audio_task owns I2S exclusively, so
 *   this either crashed or was silent during story playback.
 *   Fix: volume buttons set g_stop_flag=true and send NOTIFY_VOLUME_BIT
 *   to audio_task.  audio_task stops current audio, plays the vN.wav
 *   announcement with a local stop flag (so it can itself be interrupted),
 *   then resumes if a story was paused.
 *   Result: volume works everywhere — during stories, announcements,
 *   welcome.wav, rotate.wav, INSMUH.WAV — and pressing vol again while
 *   the vN.wav is playing interrupts it immediately.
 *
 ...
 *   bit 25  NOTIFY_VOLUME_BIT          — play volume announcement + optional resume
 */
...
/* -------------------------------------------------------------------------
 * FIX C: Volume button handler — called from main loop.
 *
 * Design:
 *   - Adjusts volume level immediately (no I2S here).
 *   - Sets g_volume_local_stop so any currently-playing vN.wav is cancelled.
 *   - Sets g_stop_flag to interrupt stories / announcements.
 *   - Cancels the encoder settle timer to prevent a stale nav announcement
 *     from racing with the volume announcement in audio_task.
 *   - Sends NOTIFY_VOLUME_BIT to wake audio_task.
 *
 * audio_task plays vN.wav using g_volume_local_stop as its stop flag, so
 * another vol press while vN.wav is playing cancels it instantly.
 * After the announcement, audio_task simply returns to the wait loop —
 * it does NOT restart any story (story restart is not needed; the user
 * pressed vol to hear the level, not to replay).
 * ---------------------------------------------------------------------- */
static void handle_volume_change(bool up) {
    ...
    g_stop_flag         = true;   /* interrupt story / any other audio */
...
static void audio_task(void *arg) {
    ...
    while (1) {
         ...
         *
         * Play vN.wav using g_volume_local_stop as the stop flag.
         * That flag is set TRUE by every vol button press, so any
         * in-progress vN.wav is cancelled the instant the next press fires.
         * We arm it to FALSE here, just before starting playback.
         *
         * Path resolution: we resolve the path inline by probing the known
         * announcement directories in priority order. This avoids calling
         * announce_check_and_play() which would queue NOTIFY_ANNOUNCE_BIT
         * and cause a re-entrant dispatch issue inside audio_task.
         *
         * After the announcement we clear g_stop_flag so navigation
         * and the next button press work normally.
         * ---------------------------------------------------------------- */
        if (nv & NOTIFY_VOLUME_BIT) {
            ...
            /* Clear stop flag so navigation / next announce works normally */
            g_stop_flag = false;
        ...
        if (nv & NOTIFY_QUIZ_BIT) {
            ...
            if (quiz_begin(g_quiz_story_path)) {
                ...
                if (g_stop_flag) {
                    ESP_LOGI(TAG, "Quiz aborted during intro");
                    g_quiz_active = false;
                    quiz_end();
                    nav_set_state(NAV_STATE_FILE_VIEW);
                } else {
                    ESP_LOGI(TAG, "Quiz ready — awaiting user selection");
                    quiz_announce_option(QUIZ_OPT_A, audio_task_handle, &g_stop_flag);
                }
            }
            nv &= ~NOTIFY_QUIZ_BIT;
            if (!nv) continue;
...
void app_main(void) {
    ...
    while (1) {
         ...
         * Interrupt-driven: works during stories, announcements, welcome.wav,
         * rotate.wav, INSMUH.WAV, etc.  A second press cancels the current
         * vN.wav announcement immediately.
         * ---------------------------------------------------------------- */
