...
void app_main(void) {
    ...
    // Initialize USB MSC (for SD card access via 2nd USB port)
    ESP_LOGI(TAG, "Attempting USB MSC initialization...");
    if (!usb_msc_init()) {
        ESP_LOGW(TAG, "USB MSC initialization failed!");
    } else {
        ESP_LOGI(TAG, "USB MSC enabled - SD card accessible via USB mass storage");
    }
    ...
    while (1) {
        ...
        // Log USB MSC status periodically (every 5 seconds)
        static uint32_t last_usb_log = 0;
        uint32_t now = xTaskGetTickCount();
        if (now - last_usb_log > pdMS_TO_TICKS(5000)) {
            last_usb_log = now;
            const char *usb_status = usb_msc_get_status();
            if (usb_msc_is_connected()) {
                ESP_LOGI(TAG, "USB Status: %s", usb_status);
            }
        }
