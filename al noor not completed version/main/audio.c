...
bool audio_play_file(const char *filepath, 
    ...
    wav_info.channels = *((uint16_t*)(header + 22));
    wav_info.sample_rate = *((uint32_t*)(header + 24));
    wav_info.bits_per_sample = *((uint16_t*)(header + 34));
    wav_info.data_size = *((uint32_t*)(header + 40));
    wav_info.data_offset = 44;
    ...
    ESP_LOGI(TAG, "Format: %lu Hz, %d-bit, %d channels", 
    ...
    while (1) {
        ...
        // Process audio based on channel count
        if (wav_info.channels == 1) {
            // Mono input -> Mono output (with volume scaling)
            ...
            // Stereo input -> Mono output (left channel only, with volume)
            int16_t *samples_in = (int16_t*)input_buffer;
            int16_t *samples_out = (int16_t*)output_buffer;
            
            for (size_t i = 0; i < frames_read; i++) {
                int16_t left_channel = samples_in[i * 2 + 0];
                int32_t scaled = ((int32_t)left_channel * vol) / 100;
                samples_out[i] = clip_sample(scaled);
            }
