 ...
 * PATCH (this revision):
 * ----------------------
 * 4. sd_scan_folders and sd_scan_subfolders now qsort their results
 *    alphabetically (case-insensitive) using compare_path_alpha.
 *    Without this, prophets and top-level folders appear in FAT32
 *    copy order, which is non-deterministic and feels random to the
 *    user when scrolling.  With this patch, the same SD card always
 *    yields the same scrolling order across reboots and across
 *    machines that recreated the card.
 */
...
/*
 * Case-insensitive path-name comparator for qsort.
 *
 * Used for top-level folders and prophet subfolders, e.g.
 *   ADAM, DAOUD, IBRAHIM, ISMAEEL, ISSA, MOUSSA, MUHAMMAD, NUH, YAQOOB, YOUSUF
 *
 * FAT32 readdir() returns entries in copy/creation order, NOT alphabetical,
 * so prophets appear in whatever order the user happened to drag them onto
 * the card.  Sorting alphabetically here gives the user a stable, predictable
 * scrolling order regardless of how the SD content was assembled.
 */
static int compare_path_alpha(const void *a, const void *b) {
    const char *pa = *(const char *const *)a;
    const char *pb = *(const char *const *)b;

    /* Compare by trailing folder name only (portion after last '/'). */
...
/* is_story_wav_file                                                   */
/*                                                                     */
/* Returns true ONLY for main navigable story files:                  */
/*   m1.wav, m10.wav, a3.wav, ib2.wav, T5.wav, T20.wav …             */
/*                                                                     */
/* Rejects ALL quiz support files:                                     */
/*   sm1_1.wav / sm1.wav   — quiz question  (starts with s + alpha)   */
/*   crrm1_1.wav / crrm1.wav — correct ans  (starts with "crr")       */
/*   wrm1_1.wav  / wrm1.wav  — wrong ans    (starts with "wr")        */
/*   CRRM1.WAV, WRM1.WAV, SM1.WAV  — same, uppercase, no underscore  */
/*                                                                     */
/* Rules (ALL must pass):                                              */
/*   1. Must end in .wav (case-insensitive)                            */
/*   2. Must NOT contain underscore  → drops s*_N, crr*_N, wr*_N     */
/*   3. Must NOT start with "crr"    → drops CRRM1.WAV etc.           */
/*   4. Must NOT start with "wr"     → drops WRM1.WAV etc.            */
/*   5. Stem (before .wav) must be:  [alpha]+ [digit]+  only          */
/*      — rejects SM1 because 's' alone before 'm' is still alpha,    */
/*        BUT 'sm' prefix is caught here: after stripping the alpha    */
/*        prefix "sm" the remainder is digits "1" which passes rule 5.*/
/*      — We add Rule 6 to handle this.                               */
/*   6. The alpha prefix must NOT be a known quiz-file prefix:         */
/*        "s"  + same story prefix  →  question file  (sa, sm, sib…)  */
/*      We detect this by checking if the prefix starts with 's'      */
/*      followed by at least one more alpha char (e.g. "sm", "sa").   */
/*      Single-letter story prefixes that start with 's' do not exist */
/*      in the current prophet table, so this is safe.                */
/*      (Prophet prefixes: a d ib is i ms m n yq y t — none is "s")  */
/* ------------------------------------------------------------------ */
static bool is_story_wav_file(const char *filename)
{
    if (!filename) return false;

    /* Rule 1: must end in .wav */
    const char *dot = strrchr(filename, '.');
    if (!dot || strcasecmp(dot, ".wav") != 0) return false;

    /* Rule 2: no underscore */
    if (strchr(filename, '_') != NULL) return false;

    /* Rule 3: must not start with "crr" (case-insensitive) */
    if (strncasecmp(filename, "crr", 3) == 0) return false;

    /* Rule 4: must not start with "wr" (case-insensitive) */
    if (strncasecmp(filename, "wr", 2) == 0) return false;

    /* Rule 5 + 6: stem must be [alpha]+ [digit]+ exactly,
     * AND the alpha prefix must not begin with 's' followed by
     * more alpha chars (which would be a quiz question prefix). */
    size_t name_len = (size_t)(dot - filename);
    if (name_len == 0) return false;

    /* Scan leading alpha characters */
    size_t i = 0;
    while (i < name_len && isalpha((unsigned char)filename[i])) i++;

    /* Must have at least one leading alpha */
    if (i == 0) return false;

    /* Rule 6: if prefix starts with 's' and has more alpha after it,
     * it is a quiz question file prefix (sm, sa, sib, sis, …) → reject */
    if (tolower((unsigned char)filename[0]) == 's' && i > 1) return false;

    /* Must have at least one digit after the alpha prefix */
    size_t digit_start = i;
    while (i < name_len && isdigit((unsigned char)filename[i])) i++;
    if (i == digit_start) return false;   /* no digits → reject */

    /* Stem must be fully consumed (no trailing junk) */
...
void sd_scan_folders(const char *path) {
    ...
    /* Sort top-level folders alphabetically so STORIES, TAWID, etc. always
     * appear in the same order regardless of how they were copied to the SD
     * card.  Hidden helper folders (e.g. announcements) are filtered later
     * by the navigation layer; sorting them here is harmless. */
    if (found > 1)
        qsort(folder_list, (size_t)found, sizeof(char *), compare_path_alpha);
...
void sd_scan_subfolders(const char *folder_path) {
    ...
    /* Sort subfolders alphabetically so prophets always appear in the same
     * order (ADAM, DAOUD, IBRAHIM, ...) regardless of FAT32 copy order.
     * This is what fixes the "stories appear in random order" symptom when
     * scrolling through prophet folders. */
    if (num_subfolders > 1)
        qsort(subfolder_list, (size_t)num_subfolders, sizeof(char *), compare_path_alpha);
