 ...
 *       A1ANN.WAV  … A10ANN.WAV
 *       D1ANN.WAV  … D10ANN.WAV
 *       IB1ANN.WAV … IB10ANN.WAV
 *       IS1ANN.WAV … IS10ANN.WAV
 *       I1ANN.WAV  … I10ANN.WAV
 *       M1ANN.WAV  … M10ANN.WAV
 *       MS1ANN.WAV … MS10ANN.WAV
 *       N1ANN.WAV  … N10ANN.WAV
 *       Y1ANN.WAV  … Y10ANN.WAV
 *       YQ1ANN.WAV … YQ10ANN.WAV
 *       v1.wav … v10.wav    (volume feedback)
 *       correct.wav, wrong.wav, b1.wav, b2.wav, b3.wav
 *       TA.wav, TT1.wav … TT20.wav
 */
 ...
 * announce_request
 *
 * Queues filepath for playback and sends NOTIFY_ANNOUNCE_BIT to audio_task.
 *
 * *** FIX (quiz option silent-audio race) ***
 *
 * ORIGINAL code had:
 *     if (stop_flag) *stop_flag = true;   <-- THIS LINE REMOVED
 *
 * Why it was there:
 *   It was meant to stop any currently-playing story/audio before the
 *   announcement began.  For navigation announcements where stop_flag
 *   points to &g_stop_flag this was harmless — g_stop_flag was already
 *   true before the call.
 *
 * Why it broke quiz option audio:
 *   In the quiz context stop_flag points to &g_quiz_announce_stop, not
 *   &g_stop_flag.  The encoder_callback sequence was:
 *     1. g_quiz_announce_stop = true   (stop old option audio)       ✓
 *     2. g_quiz_announce_stop = false  (re-arm for new audio)        ✓
 *     3. quiz_announce_option → announce_request(…, &g_quiz_announce_stop)
 *        OLD: *stop_flag = true  → g_quiz_announce_stop = true again ✗
 *     4. audio_task wakes on NOTIFY_ANNOUNCE_BIT
 *        sets *stop_to_use = false, calls audio_play_file …
 *        audio_play_file loop checks stop flag → already true → exits
 *   Result: the newly-selected option file was killed before a single
 *   sample played — silent option audio.
 *
 * The fix:
 *   Remove *stop_flag = true from here.  Every caller that needs to stop
 *   current audio must set the flag BEFORE calling announce_request /
 *   announce_check_and_play.  All navigation call sites in main.c already
 *   set g_stop_flag = true before these calls.  The quiz encoder path sets
 *   g_quiz_announce_stop = true (step 1 above) then = false (step 2)
 *   before quiz_announce_option, so the new audio flag is clean when
 *   audio_task picks it up.
 *
 *   The stop_flag parameter is kept so all existing call sites compile
...
void announce_request(const char *filepath,
    ...
    /* REMOVED: if (stop_flag) *stop_flag = true;
     * See function comment above for full explanation. */
    (void)stop_flag;
 ...
 *   Supports story numbers 1-10 (e.g. a10.wav -> A10ANN.WAV).
 *   Quiz support files (contain underscore) -> no track announcement
 */
bool announce_get_story_file(const char *filename, char *buffer, size_t buffer_size) {
    ...
    /* Reject quiz support files immediately — they always contain an underscore.
     * Do this FIRST, before any prefix matching, as a hard gate. */
    if (strchr(filename, '_')) {
        ESP_LOGD(TAG, "Quiz support file — no track announcement: %s", filename);
        return false;
    }
    ...
    if (upper[0] == 'I' && upper[1] == 'B' && isdigit((unsigned char)upper[2])) {
        story_num = atoi(&upper[2]);
        if (story_num >= 1 && story_num <= 10) {
