 ...
 * 2. FOLDER_VIEW   - Browse main folders  (STORIES, TEACHINGS …)
 ...
 *
 * FIX CRASH — USB MSC SD corruption prevention
 * -----------------------------------------------------------------------
 * usb_msc_set_sd_card() is now called at boot time BEFORE audio_task is
 * created, so there is no concurrent SPI audio access when the SD card
 * is registered with the USB host. This is safe because:
 *   - audio_task has not been created yet at this point
 *   - No playback is running
 *   - The shared SD mutex serialises any future concurrent access
 * The original corruption happened because USB MSC was registered AFTER
 * audio was already running. Boot-time registration avoids this entirely.
 * RULE: plug USB cable only when no audio is playing, and always eject
 * from PC before pressing any buttons on the device.
 *
 * FIX CRASH — headphone_detect assert crash
 * -----------------------------------------------------------------------
 * headphone_detect.c called audio_set_volume() from the hp_detect task.
 * audio_set_volume() called announce_volume_percent() → i2s_write() while
 * audio_task owned the I2S mutex, causing:
 *   assert failed: xTaskPriorityDisinherit tasks.c:5147
 * Fix is in audio.c (audio_set_volume_silent) and headphone_detect.c.
 * No changes needed in main.c for this specific fix.
 *
 * Notification bits:
...
static bool folder_is_hidden(const char *folder_name) {
    ...
    /* Hide FOUND.000 — created by Windows chkdsk after FAT corruption */
    if (strncasecmp(folder_name, "FOUND.", 6) == 0) return true;
    return false;
}

/* PATCH: Quizzes are restricted to the TEACHINGS folder only.
 * Stories of the Prophets and Invocations must never trigger a quiz
 * even if matching quiz files happen to exist on the SD card.
 *
 * UPDATED for 3-level structure (TEACHINGS/TAWHID/, TEACHINGS/PRAYER/,
 * TEACHINGS/RAMADAN/): we now scan the WHOLE path for the substring
 * "/TEACHINGS/" rather than just the immediate parent folder.  This
 * makes the gating work whether content is flat (TEACHINGS/t1.wav) OR
 * nested (TEACHINGS/TAWHID/t1.wav).  Case-insensitive match. */
static bool is_quiz_eligible_folder(const char *story_filepath) {
    if (!story_filepath) return false;

    /* Case-insensitive scan for "/TEACHINGS/" anywhere in the path.
     * We require the slashes on both sides to avoid false matches such
     * as a hypothetical "TEACHINGSX" folder. */
    const char *p = story_filepath;
    while (*p) {
        if (*p == '/' && strncasecmp(p, "/TEACHINGS/", 11) == 0) {
            return true;
        }
        p++;
    }
...
static void _announce_and_drain(const char *filename) {
    ...
    /* PATCH: suppress duplicate consecutive announcements.
     * When the user scrolls to ADAM, the settle timer plays "Adam".
     * When they then press enter on ADAM, the enter-subfolder handler
     * tries to play "Adam" again — but the user just heard it.
     * We dedupe by remembering the last filename announced and skipping
     * if it matches. Any different announcement (or a track playback in
     * between via _play_inline) resets the guard. */
    static char s_last_announced[64] = {0};
    if (s_last_announced[0] != '\0' &&
        strcasecmp(s_last_announced, filename) == 0) {
        ESP_LOGD(TAG, "Skipping duplicate announcement: %s", filename);
        return;
    }
    strncpy(s_last_announced, filename, sizeof(s_last_announced) - 1);
    s_last_announced[sizeof(s_last_announced) - 1] = '\0';
...
static void start_track_playback(int track_index) {
    ...
    if (folder_name && strcasecmp(folder_name, "TEACHINGS") == 0) {
        int tt_num = (filename &&
...
static void enter_selected_folder(void) {
    ...
    if (num_sub > 0) {
        ...
        if (num_wavs > 0) {
            ...
            if (strcasecmp(fname, "TEACHINGS") == 0)
            ...
            if (g_next_track_pending) {
                ...
                if (ti >= 0 && ti < sd_get_wav_count()) {
                    ...
                    if (fp) {
                        ...
                        if (!g_stop_flag &&
                            is_quiz_eligible_folder(fp) &&
                            quiz_exists_for_story(fp)) {
...
void app_main(void) {
    ...
    if (!sd_ok) {
        ...
        /* FIX: Register SD card with USB MSC at boot time ONLY — before
         * audio_task is created and before any playback starts.
         * This is safe because no concurrent SPI audio access exists yet.
         * The shared SD mutex in sd_card.c serialises all future access.
         *
         * IMPORTANT RULE FOR USERS:
         *   - Plug USB cable only when audio is NOT playing
         *   - Always eject the drive from PC before pressing any device buttons
         *   - Never unplug USB while PC is writing to the card */
        usb_msc_set_sd_card(sd_get_card_handle());
        ESP_LOGI(TAG, "USB MSC: SD card registered (boot-time, safe)");
        check_sd_ota_update();
    }
