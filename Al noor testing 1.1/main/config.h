...
/* If physical clockwise rotation moves the menu BACKWARDS on first hardware
 * test, change this to 1 and rebuild.  The quadrature decoder will then
 * swap CW <-> CCW so clockwise = forward (next item alphabetically). */
 ...
 *
 * Each slot costs 4 bytes (pointer) + the path string itself (~50 bytes
 * average).  500 * 54 = ~27 KB worst case in heap, which is comfortable
 * on the ESP32-S3 with ~250 KB DIRAM free.  Effectively unlimited for
 * this product's content scale (Stories, Teachings, Invocations).
 * ======================================================================== */
