...
{
    ESP_LOGW(TAG, "Rebooting into USB ROM download mode...");
    /* Restore UART log before reset so the log line is visible */
    esp_tusb_deinit_console(TINYUSB_CDC_ACM_0);
    vTaskDelay(pdMS_TO_TICKS(50));

    /* Set FORCE_DOWNLOAD_BOOT bit in RTC_CNTL_OPTION1_REG.
     * The ESP32-S3 ROM bootloader checks this on the next boot and
     * enters USB serial download mode instead of loading the app.
     * The device re-enumerates as VID:PID 0x303A:0x0002 (Espressif ROM)
     * and can be flashed with: esptool.py --chip esp32s3 -p COMx ...  */
