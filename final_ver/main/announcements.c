...
bool announce_check_and_play(const char *root_path,
    ...
    if (ann_file) {
        if (access(ann_file, F_OK) == 0) {
            ESP_LOGI(TAG, "FOUND: %s", ann_file);
            announce_request(ann_file, audio_task_handle, stop_flag);
            free(ann_file);
            return true;
        }
        ESP_LOGD(TAG, "  not at: %s", ann_file);
        free(ann_file);
    }
    ...
    if (root_file) {
        if (access(root_file, F_OK) == 0) {
            ESP_LOGI(TAG, "FOUND: %s", root_file);
            announce_request(root_file, audio_task_handle, stop_flag);
            free(root_file);
            return true;
        }
        ESP_LOGD(TAG, "  not at: %s", root_file);
        free(root_file);
    }
    ...
    if (folder_path) {
        ...
        if (folder_file) {
            if (access(folder_file, F_OK) == 0) {
                ESP_LOGI(TAG, "FOUND: %s", folder_file);
                announce_request(folder_file, audio_task_handle, stop_flag);
                free(folder_file);
                return true;
            }
            ESP_LOGD(TAG, "  not at: %s", folder_file);
            free(folder_file);
        }
    }

    ESP_LOGW(TAG, "NOT FOUND: %s (searched announcements/, root, folder)", filename);
    return false;
}
