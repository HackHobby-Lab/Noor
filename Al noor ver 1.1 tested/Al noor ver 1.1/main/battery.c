...
/* Add alongside existing playing_flag pointer */
static volatile bool *s_quiz_flag = NULL;

void battery_set_quiz_flag(volatile bool *flag) {
    s_quiz_flag = flag;
}

/* In the alert-queuing logic, suppress b1/b2/b3 during active quiz: */
static void maybe_queue_alert(const char *fname) {
    if (s_quiz_flag && *s_quiz_flag) {
        ESP_LOGI(TAG, "Battery alert suppressed during quiz: %s", fname);
        return;   /* drop alert — quiz option audio has priority */
    }
