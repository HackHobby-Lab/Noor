...
static void headphone_monitor_task(void *arg) {
    ...
    while (1) {
        ...
        if (current_state != last_state) {
            ...
            // Typical wiring: detect pin HIGH = no headphone, LOW = headphone inserted
            // Adjust this logic if your jack has opposite polarity
            if (current_state) {
                // No headphone -> enable speaker
                gpio_set_level(SPEAKER_ENABLE_PIN, 1);
                headphones_connected = false;
                ESP_LOGI(TAG, "Headphones removed -> Speaker ENABLED");
            } else {
                // Headphone inserted -> disable speaker
                gpio_set_level(SPEAKER_ENABLE_PIN, 0);
                headphones_connected = true;
                ESP_LOGI(TAG, "Headphones detected -> Speaker DISABLED");
            }
        }
...
bool headphone_detect_init(void) {
    ...
    if (HEADPHONE_DETECT_PIN < 0 || SPEAKER_ENABLE_PIN < 0) {
        ESP_LOGI(TAG, "Headphone detection disabled (pins not configured)");
        is_enabled = false;
        return false;
    }
    ...
    // Configure speaker enable pin as output
